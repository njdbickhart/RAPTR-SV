...
import stats.binary.SamBinaryConverter;
...
public class SamBinaryFileQuickSort extends TempBinaryData{
	...
	private String tempDirectory = System.getProperty("java.io.tmpdir");
        ...
        private static final Logger log = Logger.getLogger(SamBinaryFileQuickSort.class.getName());
	 ...
	 * @throws IOException
	 */
	public void splitChunks(RandomAccessFile in, String identifier){
		outputs.clear();
	 ...
	 *
	 */
	public class ComparatorDelegate implements Comparator<byte[]>{


            @Override
            public int compare(byte[] t, byte[] t1) {
                short tnum = SamBinaryConverter.getShortSAMBinFromBinary(t);
                short t1num = SamBinaryConverter.getShortSAMBinFromBinary(t1);
                long tlong = SamBinaryConverter.getCloneHashSAMFromBinary(t);
                long t1long = SamBinaryConverter.getCloneHashSAMFromBinary(t1);
                if(tlong == t1long);
                else
                    if(tnum == t1num);
                    else
                        return Integer.compare(tnum, t1num);
                
                return Long.compare(tlong, tlong);
            }
